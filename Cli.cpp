...
bool Cli::runGame() {
    ...
    while (inputCheck && !exit) {
        if ((input.length() != 1 ||
            validInputs.find(input) == std::string::npos) && !exit) {
            std::cout << "\nInvalid Input\n" << std::endl;
            std::cout << "> ";
            input = getInput();
    ...
    if (input.size() == 4) {
        if (input[0] == "place" && input[2] == "at") {
            if (validateTile(input[1]) && validatePosition(input[3])) {
                ...
                bool validMove =
                        gameEngine->checkTilePlacement(player, row, col,
                                                       &tileToPlace);
                if (validMove) {
                    gameFinished=gameEngine->endOfRoundCalculations(player,
                                                                  row,
                                                                  col,
                                                                  &tileToPlace);
                    status = true;
