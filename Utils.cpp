...
void utils::updateScores(Player* player, Board* board, int row, int col, bool initialTilePlaced) {
...
int utils::calculateLineScore(Board* board, int row, int rowIncrement, int col, int colIncrement) {
    int score = 0;
...
void utils::adjacentTileChecker(bool& validMove, bool& colourMatch, bool& shapeMatch, Tile* tile1, Tile* tile2, Tile* toAddTile) {
    if (tile1 == nullptr && tile2 == nullptr) {
        ...
        if ((tile1->colour == toAddTile->colour && tile2->colour == toAddTile->colour)) {
            validMove = true;
            colourMatch = true;
        } else if (tile1->shape == toAddTile->shape && tile2->shape == toAddTile->shape) {
            validMove = true;
            shapeMatch = true;
        }
    }
