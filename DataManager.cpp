...
bool DataManager::saveGame(GameState* saveGame, std::string fileName) {
    ...
    if (loadFile.is_open()) {
        ...
        for (int i = 0; i < NUMBER_OF_DATA_ENTRIES && validFile; ++i) {
    ...
    if (validFile) {
        ...
        if (saveFile.is_open()) {
            // check file is valid file to save to.
...
GameState* DataManager::loadGame(std::string fileName) {
    ...
    if (loadFile.is_open()) {
        // initialise temp variables
        std::string loadedData[NUMBER_OF_DATA_ENTRIES];
        for (int i = 0; i < NUMBER_OF_DATA_ENTRIES && dataValid; ++i) {
            std::getline(loadFile, loadedData[i]);
            loadedData[i] = removeCarriageReturn(loadedData[i]);
            utils utils;
            dataValid = utils.checkData(i, loadedData[i]);
        }
